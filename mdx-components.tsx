...
export function useMDXComponents(components: MDXComponents): MDXComponents {
  return {
    ...
    h1: (props) => (
      <h1
        className="mb-6 mt-2 font-sans text-2xl font-medium uppercase tracking-[0.2em] text-foreground lg:text-3xl"
        {...props}
      />
    ),
    h2: (props) => (
      <h2
        className="mb-4 mt-8 font-sans text-lg font-medium uppercase tracking-[0.15em] text-foreground first:mt-0"
        {...props}
      />
    ),
    h3: (props) => (
      <h3
        className="mb-3 mt-6 font-sans text-base font-medium uppercase tracking-[0.1em] text-foreground/90"
        {...props}
      />
    ),
    h4: (props) => (
      <h4
        className="mb-2 mt-4 font-sans text-sm font-medium uppercase tracking-[0.1em] text-foreground/80"
        {...props}
      />
    ),
    ...
    p: (props) => (
      <p
        className="mb-4 font-sans text-sm leading-relaxed text-foreground/85 last:mb-0"
        {...props}
      />
    ),
    strong: (props) => (
    ...
    pre: (props) => (
      <NierWindow title="Code">
        <pre
          className="overflow-x-auto font-mono text-[13px] leading-relaxed text-foreground/85"
          {...props}
        />
      </NierWindow>
    ),
    ...
    thead: (props) => (
      <TableHeader className="bg-accent/40">{props.children}</TableHeader>
    ),
    tbody: (props) => <TableBody>{props.children}</TableBody>,
    tr: (props) => (
      <TableRow className="border-border/20 hover:bg-accent/20">
        {props.children}
      </TableRow>
    ),
    th: (props) => (
    ...
    td: (props) => (
      <TableCell className="font-sans text-sm text-foreground/85">
        {props.children}
      </TableCell>
    ),
