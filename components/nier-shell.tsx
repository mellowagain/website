...
function getActiveId(pathname: string) {
    ...
    const match = menuItems.find((item) => item.id !== "/" && pathname.startsWith(item.id));
    return match?.id ?? "/";
}
        ...
        <div className="flex h-full items-center justify-center">
            <div className="flex flex-col items-center gap-6">
                ...
                <div className="flex items-center gap-1">
                    <span className="font-sans text-xs uppercase tracking-[0.3em] text-muted-foreground/60">Loading</span>
                    <span className="w-5 font-mono text-xs text-muted-foreground/60">{".".repeat(dots)}</span>
                </div>
            </div>
        </div>
    );
            ...
            <div className="relative z-10 flex flex-1 overflow-hidden px-6 pt-8 md:px-16 md:pt-10 lg:px-32 lg:pt-12 xl:px-52 xl:pt-14 2xl:px-80 2xl:pt-16">
                ...
                <div className="hidden shrink-0 flex-col md:flex md:w-56 lg:w-64">
                    ...
                    <div className="flex-1 overflow-y-auto">
                        <NierMenu activeSection={activeId} onSectionChange={handleNav} items={menuItems} />
                    </div>
                    ...
                    <div className="flex flex-1 flex-col overflow-hidden border border-border/40 bg-card/40">
                        ...
                        <div className="flex items-center gap-2.5 border-b border-border/30 bg-accent/50 px-5 py-2.5">
                            <span
                                ...
                                aria-hidden="true"
                            />
                            <span className="font-sans text-xs font-light uppercase tracking-[0.25em] text-foreground/70">{pageLabel}</span>
                        </div>
                        ...
                        {showLoading ? (
                            ...
                            <NierScrollArea className="px-6 py-6 md:px-8 md:py-8 lg:px-10 lg:py-10">{children}</NierScrollArea>
                        )}
                    </div>
                </main>
            </div>
